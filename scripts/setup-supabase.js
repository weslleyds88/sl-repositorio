...
const createTables = async (supabase) => {
  try {
    // Tentar executar schema SQL via RPC
    ...
    for (const statement of statements) {
      if (statement.trim()) {
        try {
          // Tentar executar via RPC (pode não funcionar com anon key)
          const { data, error } = await supabase.rpc('exec_sql', {
            sql: statement + ';'
          });

          if (error) {
            console.log(`⚠️  Não foi possível executar via API: ${error.message}`);
            console.log('📋 Execute o SQL manualmente no Supabase Dashboard');
            console.log('   SQL Editor > Cole o conteúdo do arquivo sql/schema.sql');
            return false;
          }
        } catch (err) {
          console.log(`⚠️  Erro na statement: ${err.message}`);
        }
      }
    }

    console.log('✅ Schema executado com sucesso via API!');
    return true;

  } catch (error) {
    console.log('⚠️  Erro ao executar schema via API:', error.message);
    console.log('📋 Execute o SQL manualmente no Supabase Dashboard');
    console.log('   SQL Editor > Cole o conteúdo do arquivo sql/schema.sql');
    return false;
  }
